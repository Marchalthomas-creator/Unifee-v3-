...
export default function SimulationGazPage() {
  ...
  const [aboUnifee, setAboUnifee] = useState("90");
  ...
  useEffect(() => {
    ...
    try {
      ...
      setAboUnifee(settings.gazAbo || "90");
    } catch {
      // no-op
    }
  }, []);
  ...
  function calculer() {
    ...
    const prixActuel = toNumber(prixKwh);
    const aboActuel = toNumber(abonnement);

    const prixU = toNumber(prixUnifee);
    const aboU = toNumber(aboUnifee);

    const totalActuelHT = conso * prixActuel + aboActuel;
    const totalUnifeeHT = conso * prixU + aboU;

    const coefficientReglemente = 1.10;
        ...
        <div className="space-y-5 rounded-[30px] border border-slate-200/80 bg-white/95 p-5 shadow-[0_12px_40px_rgba(15,23,42,0.08)] backdrop-blur">
          <div className="text-center">
            ...
            <h1 className="mt-2 text-3xl font-bold text-slate-900">
              Comparez l’offre actuelle avec UNIFEE
            </h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Remplissez les données du client ou utilisez l’extraction automatique.
            </p>
          </div>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-gradient-to-br from-slate-50 to-emerald-50/60 p-5">
            ...
            <button
              ...
              className="mt-4 w-full rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-3 font-semibold text-white shadow-sm transition hover:from-emerald-700 hover:to-teal-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          <section className="space-y-4 rounded-[24px] bg-gradient-to-br from-slate-50 to-emerald-50 p-5">
            <h2 className="text-2xl font-bold text-slate-900">
              Votre offre UNIFEE
            </h2>
